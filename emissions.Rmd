...
* Concentrations of hazardous air pollutants and associated risks are highest in the eastern half of the state and around the urbanized areas in central and western Vermont. 
...
exPM25town <- vt_towns %>% as.data.frame() %>% filter(pctrank_PM25_19.mean > 95) %>% select(NAME) %>% slice_tail(n = 1) %>% pull()
exPM25 <- vt_towns %>% as.data.frame() %>% filter(NAME == exPM25town) %>% select(PM25_19.mean) %>% pull() %>% round(., 3)
exPM25pct <- vt_towns %>% as.data.frame() %>% filter(NAME == exPM25town) %>% select(pctrank_PM25_19.mean) %>% pull() %>% round(., 1)
