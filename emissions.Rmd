...
exPM25 <- vt_towns %>% as.data.frame() %>% filter(NAME == exTown) %>% select(PM25_19.mean) %>% pull() %>% round(., 3)
exPM25pct <- vt_towns %>% as.data.frame() %>% filter(NAME == exTown) %>% select(pctrank_PM25_19.mean) %>% pull() %>% round(., 1)
